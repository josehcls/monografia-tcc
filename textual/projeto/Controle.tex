...
É importante destacar que o dispositivo tem duas funções: a primeira é manter a temperatura do líquido fermentado; a segunda, levar o líquido até determinada temperatura. Ambas as funcionalidades envolvem a capacidade do dispositivo de retirar ou fornecer calor do sistema de forma eficiente e constante. 
...
\begin{enumerate}
    ...
    \item A condução é unidimensional no plano horizontal
...
\begin{figure}[h]
    ...
    \captionsource{Malha de controle PID.}{http://microcontrolado.com/}
    \label{fig:pid}
\end{figure}
...
\begin{figure}[h]
    ...
    \captionsource{Responda do Sistema em malha aberta a sinal degrau.}{https://pt.wikipedia.org/}
...
Nesta aplicação, devido a limitações do atuador, é esperado uma saturação na saída. No caso onde o sistema não consegue chegar no \textit{setpoint}, ocorre uma situação chamada de wind-up, na qual, a sua resposta integral irá crescer de forma indefinida. Para evitar problemas no controlador, é necessário a implementação de um sistema anti wind-up, no caso, a ação escolhida é congelar a ação do controlador em caso de saturação.  
